...
func main() {
	ctx := logging.NewDefaultLogger(context.Background(), logging.WithService("bookings-migrate"))
	log.Ctx(ctx).Info().Msg("starting bookings migrate lambda")
	...
	lambda.Start(func(ctx context.Context) error {
		...
		tmpDir, err := os.MkdirTemp("", "bookings_migrations")
		if err != nil {
			return err
		}
