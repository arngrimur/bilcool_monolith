...
func main() {
	ctx := logging.NewDefaultLogger(context.Background(), logging.WithService("journal-migrate"))
	log.Ctx(ctx).Info().Msg("starting journal migrate lambda")
	...
	lambda.Start(func(ctx context.Context) error {
		...
		tmpDir, err := os.MkdirTemp("", "journal_migrations")
		if err != nil {
			return err
		}
