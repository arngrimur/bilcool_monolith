...
func main() {
	ctx := logging.NewDefaultLogger(context.Background(), logging.WithService("authentication-outbox"))
	log.Ctx(ctx).Info().Msg("starting authentication outbox lambda")
	...
	p := poller.New(db, publisher, snspublisher.TopicUsers)
